<template>
  <v-container class="d-none d-lg-block pb-0">
    <!-- 麵包屑 -->
  ...
  <v-row style="height: 95vh;">
    ...
    <v-col cols="12" lg="4" class="map_col order-1 order-lg-2 d-none d-lg-block">
  ...
  <v-bottom-sheet v-model="sheet" persistent class="d-lg-none">
    <template v-slot:activator="{ props }">
      <v-btn
        ...
        dark
        v-bind="props"
        class="bg-accent add-resource-btn d-lg-none font-weight-bold"
      >
        新增資源
      </v-btn>
    </template>
    <v-card>
      <v-toolbar color="secondary bb-1" prominent>
        <v-toolbar-title>新增資源</v-toolbar-title>
        <v-spacer></v-spacer>
      ...
      <v-card-text class="pb-0">
        <v-form @submit.prevent="submit" :disabled="isSubmitting">
          <Search class="mx-auto my-5 text-center ps-8" 
          max-width="1200px"
          v-model="address"
          :error-messages="addressError "
          label="輸入資源地址"
          ...
          <v-row class="my-5 ">
              ...
              <v-col cols="9" md="10" class="my-auto">
                <v-select
                :items="clients"
                label="服務類別"
                density="comfortable"
              ...
              <!-- 服務項目 -->
              <v-col cols="3" md="2" class="mt-2 text-left ps-6 text-center">
                <label class="form-label">服務項目</label>
              </v-col>
              <v-col cols="9" md="10">
                <v-expansion-panels variant="accordion">
              ...
              <!-- 需求介紹 -->
              <v-col cols="3" md="2" class="ps-6 text-center">
                  <label class="form-label">服務介紹</label>
              </v-col>
              <v-col cols="9" md="10">
                <v-textarea 
          ...
          <v-row dense>
            <div style="margin: auto;" >
              <AppButton type="submit" class="b-1 bg-accent  mb-8" @click="addMarker"></AppButton>
            </div>
          </v-row>
        </v-form>
      </v-card-text>
...
// 將輸入的地址字符串中的「臺北市」替換為「台北市」
const normalizeAddress = (value) => {
  if (value) {
    address.value = value.replace(/臺北市/g, '台北市');
  }
};
...
const loadMap = async () => {
  try {
    ...
    marks.value.splice(0, marks.value.length, ...data.result.data); // 更新前端的地標
