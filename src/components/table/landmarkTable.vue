          ...
          <v-row>
            <v-col class="text-center" >
              <div class="mx-auto mb-8">
                <v-btn variant="text"class="rounded-xl b-1 bg-accent" density="comfortable" type="submit" @click="editMarker">送出</v-btn>
                <v-btn
                text="取消"
                variant="text"
                class="rounded-xl b-1 bg-info ml-2"
                density="comfortable"
                @click="closeDialog()"
                ></v-btn>
              </div>
...
const schema = yup.object({
  ...
  tel: yup
    .string(),
  category1:yup.array().of(yup.string())
    .required('分類必填'),
...
const submit = handleSubmit(async (values) => {
  try {
    await editMarker() // 避免取到經緯度時就已經執行了submit()，沒有這行會取不到lat、lng
